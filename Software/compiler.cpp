...
https://github.com/gitliuyuhan/compiler
开源软件万岁！
...
string  asmfile(string source)
{
	if(source.size()==0)
	{
		cout<<"Source file name cannot be empty"<<endl;
		exit(-1);
	}
	string temp="";
	int i,j;
...
int word_token(string s)
{
	...
	else if(isdigit(s[0]))
	{
		for(int i=1;i<size;i++)
		{
			if(!isdigit(s[i]))
			{
				cout<<"Illegal identifier:"<<s<<endl;
				exit(-1);
			}
		}
		return K_DIGIT;
		...
		default://判断单词类型
			...
			while(rfile>>try_ch)
			{
				if(try_ch == '\"')
				{
					...
					{
						cout<<"Illegal identifier:"+temp<<endl;
						exit(-1);
					}
				}
...
void if_analysis(vector<IDwords>::iterator &it)
{
	...
	if(it->word != "(")
	{
		cout<<"Bad if statement: missing '('"<<endl;
		exit(-1);
	}
	it++;
	dst = it->word;
	...
	if(it->word != ")")
	{
		cout<<"Bad if conditional statement: missing ')'"<<endl;
		exit(-1);
	}
	it++; //略过‘{’
	it++;
...
void if_asm(ofstream &out,string dst,string dsc,string mark,string step)
{
	...
	{
		cout<<"Other conditional judgments are not currently supported."<<endl;
		exit(-1);
	}
}
...
void create_asm(string file)
{
	...
	if(!wfile.is_open())
		cout<<"Unable to create assembly file"<<endl;
    ...
    for(it = target_code.begin();it != target_code.end();it++)
	{
		...
		{
			cout<<"else no matching found if"<<endl;
			exit(-1);
		}
		//跳转语句
		else if(it->op == "jmp")
		...
		{
			cout<<"The compiler does not currently support this syntax operation"<<endl;
			exit(-1);
		}
	}
...
int main(int argc,char* argv[])
{
	...
	{
		cout<<"*****************************************************"<<endl;
		cout<<"\nGenerate .asm assembly file in the source file directory\n"<<endl;
		cout<<"*****************************************************"<<endl;
	...
	{
		cout<<"Use the correct format: compile.exe [source file] [compiled file]"<<endl;
	}
