...
inline void init(){
	bool de_1=1;
	while(1){
		if(de_1)cout<<"Hitmux 1.1.0-bate (C) 2024 CaoKai,All right reserved.\n";
		cout<<"1.Hit mode.\n"<<"2.User mode.\n"<<"3.Visitor mode\n";
		string de_start="\0";
		cin>>de_start;
		string passwd="\0";
...
inline void calc_f(string a1,string a2,string a3){
	cout<<"Powerful NB calculator 1.0-bate (C) 2024 CaoKai,All right reserved.\n";
	long double n1=atof(a1.c_str()),n2=atof(a3.c_str()),ans;
	if(a2=="-a") ans=n1+n2;
	else if(a2=="-s") ans=n1-n2;
	else if(a2=="-m") ans=n1*n2;
	else if(a2=="-d") ans=n1/n2;
	else if(a2=="-p") ans=pow(n1,n2);
	else if(a2=="-log") ans=(log(n2)/log(n1));
	else if(a2=="-sin") ans=sin(n1);
	else if(a2=="-cos") ans=cos(n1);
	else if(a2=="-tan") ans=tan(n1);
	cout<<"Calculation result:   "<<fixed<<ans<<"\n";
}
...
int main(){
	...
	while(true){
	    ...
	    if(a[0]=="pwd"){
			...
			else if(a[1]=="cal") cout<<"Powerful NB calculator (C) 2024 CaoKai,All right reserved.\n<cal> <first number> <operator> <second number>\nA powerful veryNB calculator.\noperator:\n    -a:Addition.\n    -s:Subtraction.\n    -m:Multiplication.\n    -d:Division\n    -p:Power    -log:Logarithm.\n    -tan:Tangent.\n    -sin:Sine.\n    -cos:Cosine.\n{If you want to calculate trigonometric functions (sin, cos, tan), you only need to enter the first number!}";
